...
# Pin configuration
GPIO_PIN = 18  # Choose the GPIO pin you want to control (GPIO18 in this case)

# Setup
GPIO.setmode(GPIO.BCM)  # Set the pin numbering system to BCM (Broadcom)
...
def play_dtmf_and_generated_voice():
    ...
    """
    dtmf_tones = ['Dtmf-0.mp3', 'Dtmf-9.mp3', 'Dtmf-9.mp3']
    mensaje_generado = 'mensaje.wav'
    #dtmf0 = 'Dtmf-0.wav'
    #dtmf9 = 'Dtmf-9.wav' 

    # Turn GPIO pin ON (set to HIGH)
    print(f"Prendiendo el relay en el pin {GPIO_PIN}")
    GPIO.output(GPIO_PIN, GPIO.HIGH)
    time.sleep(3)
    """
    play_audio_sequence(dtmf0)
    time.sleep(1)
    play_audio_sequence(dtmf9)
    time.sleep(1)
    play_audio_sequence(dtmf9)
    time.sleep(2)
...
def escuchar_teclado_y_procesar():
    """
    Escucha indefinidamente en el puerto serial y procesa el mensaje cuando llega un string.
    """
    # Configurar el puerto serial (ajusta el puerto y baudrate a tu configuración)
